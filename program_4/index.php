...
$.get( "http://mwsu-webdev.xyz/api/api.php/products?order=id&page=1,10")
  .done(function( data ) {
	...
	{
		var productImage=rows[i][4];
		productImage=productImage.replace(/~/,'75');
		row= row + "<td>" + rows[i][0] +"</td>" + "<td>" +rows[i][1]+"</td>"+ "<td>" + rows[i][2] +"</td> <td>" + rows[i][3] + "</td> <td><img src=" + productImage + "></td></tr>";
		$('tbody').append(row);
		row="<tr>";
	}
